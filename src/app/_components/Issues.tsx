    ...
    <section className=" min-h-screen w-full">
      ...
      <div className="w-full px-4 py-8 overflow-x-auto">
        <div className="max-w-7xl mx-auto flex gap-4 sm:flex-nowrap md:grid md:grid-cols-4 lg:grid-cols-5 justify-start">
          {issues.map((issue, idx) => (
            <div
              key={issue.key}
              className={`font-heading relative min-w-[240px] md:w-auto h-20 font-bold uppercase flex items-center justify-center transition-all
                duration-300 cursor-pointer rounded overflow-hidden ${
        idx === activeIndex
      ...
      <div className="md:hidden flex justify-between items-center bg-indigo-950 py-6 px-8">
        <button
          ...
          onClick={() => setActiveIndex((activeIndex - 1 + issues.length) % issues.length)}
        >
          <ChevronLeftIcon className="w-8 h-8" />
        </button>
        <button
          className="text-white"
          onClick={() => setActiveIndex((activeIndex + 1) % issues.length)}
        >
